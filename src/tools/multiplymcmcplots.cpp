...
int main(int argc, char **argv) {
  ...
  double *pdResultX, *pdResultY, *pdResultZ;
  ...
  //      printf("%e %e %e\n", pdBufferX[i], pdBufferY[i], pdBufferZ[i]);
    ...
    double dMin=pdResultZ[0];
    double dMax=pdResultZ[0];
    double dArea=0;
    for(int i=1; i<nCount; i++) {
      if( (pdResultY[i] < 2.8 && pdResultY[i] > 2.0) || (! bBoundExp) ) {
        if(pdResultZ[i] < dMin) dMin = pdResultZ[i];
        if(pdResultZ[i] > dMax) dMax = pdResultZ[i];
        dArea += pdResultZ[i];
      } // if pdResultY
    } // for i
    ...
    for(int i=0; i<nCount; i++) {
      if(bExponentiate) {
        pdResultZ[i] = exp(dMin-pdResultZ[i]);
      } else if(bUnitize) {
        pdResultZ[i] = pdResultZ[i]/dMax;
      } else if(bNormalize) {
        pdResultZ[i] = pdResultZ[i]/dArea;
      }
    } // for i
// 0.011109, 0.043937, 0.13534, 0.32465, 0.60653, 0.88250
    ...
    for(int i=0; i<nCount; i++) {
      ...
      fprintf(pFileOut, "%.18e %.18e %.18e\n", pdResultX[i], pdResultY[i], pdResultZ[i]);
    } // for i
    ...
    QuickSort(pdResultX, pdResultY, pdResultZ, pdResultZ, 0, nCount-1);
